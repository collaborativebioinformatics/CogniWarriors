...
def _load_image_embeddings():
    frames = []
    for filename in config.IMAGE_EMBEDDING_FILES:
        df = pd.read_csv(config.IMAGE_EMBEDDING_DIR / filename)
        frames.append(df)
...
def load_multimodal_dataframe():
    ...
    """
    phenotype = pd.read_csv(config.PHENOTYPE_FEATURES_TSV, sep="\t")
    with open(config.PHENOTYPE_FEATURE_MANIFEST) as f:
    ...
    images, image_cols = _load_image_embeddings()

    df = phenotype.merge(
        composite[config.ID_COLS + config.TARGET_COLUMNS], on=config.ID_COLS, how="inner"
    )
    ...
    df = df.dropna(subset=config.TARGET_COLUMNS, how="all").reset_index(drop=True)
