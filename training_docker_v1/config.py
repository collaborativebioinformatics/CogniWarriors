...
def _default_data_root():
    """Prefer the local training/data directory, with the old repo layout as a fallback."""
    local_data = Path(__file__).resolve().parent / "data"
    if local_data.exists():
        return local_data
    parents = Path(__file__).resolve().parents
    if len(parents) > 2:
        return parents[2] / "data"
    return local_data


DEFAULT_DATA_ROOT = _default_data_root()
DATA_ROOT = Path(os.environ.get("NBBH_DATA_ROOT", DEFAULT_DATA_ROOT)).resolve()
