    ...
    while flare.is_running():
        ...
        for _ in range(args.local_epochs):
            ...
            for batch in sampler:
                ...
                batch_mse = masked_mse(preds.detach(), y).item()
                if model.lmmnn is not None:
                ...
                loss.backward()
                optimizer.step()
                train_mses.append(batch_mse)
                objective_losses.append(loss.item())

        metrics["train_loss"] = float(np.mean(train_mses))
        metrics["train_objective"] = float(np.mean(objective_losses))
        print(
            f"[{site}] round {round_num}: global val MSE {global_val_mse:.4f}, "
            f"local train MSE {metrics['train_loss']:.4f}, objective {metrics['train_objective']:.4f}"
        )
